// Generated from web/speech.ts; regenerate before deployment.
// Prefer punctuation and linguistic word boundaries; never cut by a fixed character count.
export function speechChunks(text, continuous = false) {
    ...
    while (remaining) {
        ...
        if (!end) {
            const before = marks.filter(x => x <= max);
            end = before.at(-1);
        }
        if (remaining.length <= max && !end)
        ...
        if (remaining.length - end <= 12 && remaining.length <= max)
            end = remaining.length;
...
export async function* speechMedia(response) {
    if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        throw new Error(data.detail || '声音服务不可用。');
    }
    if (!response.body || !response.headers.get('content-type')?.startsWith('text/event-stream'))
        throw new Error('声音服务没有返回分段播放流。');
    ...
    try {
        while (true) {
            ...
            if (part.done)
                throw new Error('声音传输提前结束，请重新发送。');
            buffer += decoder.decode(part.value, { stream: true });
            if (buffer.length > 72 * 1024 * 1024)
                throw new Error('声音片段过大。');
            let boundary;
            while ((boundary = buffer.indexOf('\n\n')) !== -1) {
                ...
                if (value.index !== index++ || typeof value.data !== 'string' || typeof value.mime !== 'string' ||
                    !/^(audio\/|video\/mp4)/.test(value.mime))
                    throw new Error('声音片段顺序或格式异常。');
                const binary = atob(value.data), bytes = Uint8Array.from(binary, c => c.charCodeAt(0));
                if (!bytes.length)
                    throw new Error('声音片段为空。');
...
export class Speaker {
    ...
    preloaded = new Map();
    preload(url) {
        if (this.preloaded.has(url) || this.url === url)
            return;
        ...
        while (this.preloaded.size > 2) {
            const key = this.preloaded.keys().next().value;
            const old = this.preloaded.get(key);
            old.src = '';
            old.load();
    ...
    async play(text, teacherId, mode, ended, error, continuous = false, ready) {
        this.stop();
        const generation = this.generation;
        if (continuous)
            this.onProgress(text, 0);
        const finish = () => {
            ...
            if (continuous)
                this.onProgress(text, text.length);
            this.speaking = false;
            this.onChange(false);
            ended();
        };
        const fail = (message) => { if (generation !== this.generation)
            return; this.stop(); error(new Error(message)); };
        if (mode === 'silent') {
            finish();
            return;
        }
        if (mode === 'browser') {
            if (!window.speechSynthesis) {
                fail('浏览器不支持朗读，请使用 Chrome 或配置 GPU 声音。');
                return;
            }
            ...
            utterance.onstart = () => { if (generation === this.generation) {
                this.speaking = true;
                this.onChange(true);
            } };
            utterance.onboundary = e => { if (continuous && generation === this.generation)
                this.onProgress(text, e.charIndex); };
            utterance.onend = finish;
            utterance.onerror = () => fail('浏览器朗读未完成。课程已暂停，可重新播放或切换静音阅读。');
        ...
        const dispose = (item) => { item.media.onended = null; item.media.onerror = null; item.media.ontimeupdate = null; item.media.pause(); item.media.src = ''; URL.revokeObjectURL(item.url); prepared.delete(item); };
        try {
            const chunks = ready ? [text] : speechChunks(text, continuous);
            if (!chunks.length) {
                finish();
                return;
            }
            ...
            const next = async () => {
                try {
                    const result = ready ? { done: false, value: null } : await stream.next();
                    if (result.done)
                        return result;
                    if (signal.aborted)
                    ...
                    if (ready)
                        this.preloaded.delete(ready.url);
                    const item = { media, url: ready?.url || URL.createObjectURL(result.value) };
                    prepared.add(item);
                    if (isVideo)
                        media.playsInline = true;
                    media.preload = 'auto';
                    if (media.getAttribute?.('src') !== item.url)
                        media.src = item.url;
                    ...
                    await new Promise((resolve, reject) => {
                        ...
                        const bad = () => { cleanup(); reject(new Error('下一段声音或画面加载失败。')); };
                    ...
                    return { done: false, value: { ...item, isVideo } };
                }
                catch (e) {
                    return { error: e };
                }
            };
            let pending = next(), previous = null;
            for (let i = 0; i < chunks.length; i++) {
                ...
                if ('error' in result)
                    throw result.error;
                if (result.done)
                    throw new Error('老师的回答未播放完整，请重试。');
                if (i + 1 < chunks.length)
                    pending = next();
                const item = result.value, audio = item.media;
                this.url = item.url;
                this.audio = audio;
                const prefix = chunks.slice(0, i).join('').length;
                const progress = () => {
                    if (continuous && generation === this.generation && Number.isFinite(audio.duration) && audio.duration > 0) {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    let starting = false;
                    const attempt = () => {
                        if (starting || generation !== this.generation)
                            return;
                        starting = true;
                        ...
                        audio.play().then(() => {
                            ...
                            this.onVideo(item.isVideo ? audio : null);
                            if (previous)
                                dispose(previous);
                            previous = item;
                            this.resumePlayback = null;
