...
export function LessonBoard({slide,offset,all=false}:{slide:Slide;offset:number;all?:boolean}){
 ...
 return <section className="lesson-board" aria-label="同步课堂板书"><header><span>随讲板书</span><span>想一想 · 说理由</span></header><h2>{slide.title}</h2><div className="board-writing">{slide.bullets.map((b,i)=><div key={i} className={`board-line ${visible[i]?'written':''} ${active===i?'current':''}`} aria-hidden={!visible[i]}><span className="board-number">{i+1}</span><p><Maths text={b}/></p></div>)}</div><footer className="company-signature">北京简融易数科技有限公司</footer></section>;
}
