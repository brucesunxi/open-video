...
export function Realtime({teacher,avatar,config}:{teacher:Teacher;avatar?:Asset;config:Config|null}){
  ...
  return <section className="realtime panel">
    ...
    <div className="realtime-grid"><div className={`realtime-stage ${avatar?.filename.startsWith('站姿')?'standing-conversation':''}`}><Avatar video={video} url={avatar?.url} kind={avatar?.kind} speaking={speaking} gesture={phase==='thinking'?'think':'idle'}/><h3>{teacher.name}</h3><p className="live-partial">{partial||({listening:'请说话，我在听…',thinking:'正在组织回答…',speaking:'想插话时，点击“打断并说话”',synthesizing:'正在准备下一段声音与画面，可以点击打断取消',idle:'点击开始对话，允许使用麦克风',paused:'可以重新开始，或输入文字'}[phase])}</p>
      ...
      <div className="actions realtime-controls"><button className="primary" onClick={()=>void (active.current?stop():begin())}>{active.current?<><Square size={16}/>结束对话</>:<><Mic size={16}/>开始实时对话</>}</button><button disabled={!['thinking','speaking','synthesizing'].includes(phase)} onClick={()=>void interrupt()}><Hand size={16}/>打断并说话</button><span className="realtime-credit company-signature">北京简融易数科技有限公司</span></div>
