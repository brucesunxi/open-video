...
function App(){
  ...
  return <div className={`app ${page==='classroom'&&classMode==='lesson'?'lesson-focus':''}`}>
    ...
    <div className="main"><header><div><span>工作空间</span><ChevronRight size={14}/><strong>{pages.find(x=>x[0]===page)?.[1]}</strong></div><div className="header-right"><span className="status-dot"/>本地工作台 <span className="divider"/> <span className="mode-tag">{config?.llm.configured?'模型已配置':'资料引用模式'}</span>{config?.auth_enabled&&<button disabled={busy} onClick={()=>void run(async()=>{stop();setClassMode('lesson');await post('/logout');setToken('');setAuthorized(false);})}>退出登录</button>}</div></header>
      <main>
        ...
        {page==='classroom'&&teacher&&classMode==='realtime'&&<Realtime key={`${teacher.id}:${teacher.avatar_asset_id}`} teacher={teacher} avatar={avatar} config={config}/>}
        {page==='classroom'&&teacher&&classMode==='lesson'&&<>
          ...
          <div className="class-grid"><section className="stage-column"><div className="stage panel"><div className="stage-top"><span><span className={`status-dot ${speaking?'pulse':''}`}/>{preparingSpeech?(readyPages[session?.slide_index||0]?'正在加载已准备的视频':'正在生成老师声音与画面'):session?labels[session.state]:(readyPages[0]?'视频已准备好 · 点击播放':'课堂准备中')}</span><span className="pill pale">{course?`${(session?.slide_index||0)+1} / ${course.slides.length}`:"自由问答"}</span></div>
            <div className="teaching-scene"><div className={`lecture-composition ${avatar?.filename.startsWith('站姿')?'standing-scene':''}`}><div className={`presenter ${avatar?.filename.startsWith('站姿')?'standing-presenter':''}`}><Avatar video={video} url={avatar?.url} kind={avatar?.kind} speaking={speaking} gesture={session?.state==='ANSWERING'?'think':slide?.gesture||'idle'}/><div className="presenter-name">{teacher.name}</div></div>{slide&&(!avatar?.filename.startsWith('站姿')||boardOffset>=0||voice==='silent')&&<LessonBoard slide={slide} offset={boardOffset} all={voice==='silent'} embedded={avatar?.filename.startsWith('站姿')}/> }</div>
            </div>
          ...
          <details className="lesson-extra"><summary>讲稿与课堂设置</summary><div className="lesson-extra-body"><div className="actions"><button className="text-btn" disabled={busy||!session} onClick={()=>void run(async()=>{stop();if(session&&session.state!=='FINISHED')await perform('finish');setSession(null);current.current=null;})}><RotateCcw size={15}/>新会话</button>
            ...
            <div className="caption">{caption||slide?.narration||'选择课程后即可查看讲稿。'}</div>
            {course&&<CourseMedia key={`${course.id}:${teacher.voice_profile_id}:${teacher.avatar_asset_id}`} course={course} teacher={teacher}/>}
            {sessions.some(s=>s.state!=='FINISHED')&&!session&&<button onClick={()=>void run(()=>restore(sessions.find(s=>s.state!=='FINISHED')!))}><RotateCcw size={15}/>恢复上次课堂</button>}
          </div></details></section>
          <details className="lesson-questions"><summary>有问题？向老师提问{session?.messages.length?` · ${session.messages.length} 条消息`:''}</summary>
        ...
        {page==='teachers'&&(teacher||editingTeacher)&&<div className="editor-grid"><section className="panel form-panel"><div className="section-title"><h2>{editingTeacher?.id===''?'添加老师':'教师档案'}</h2><GraduationCap size={22}/></div>
          <form onSubmit={e=>{e.preventDefault();void run(async()=>{const t=editingTeacher||teacher!;const saved=await api<Teacher>(t.id?`/teachers/${t.id}`:'/teachers',{method:t.id?'PUT':'POST',body:JSON.stringify(t)});await bootstrap();setTeacherId(saved.id);setEditingTeacher(null);setNotice('教师档案已保存。');});}}>
            ...
            <fieldset className="avatar-gallery"><legend>选择老师形象</legend><p className="subtle">点击预览图选择，再保存档案。坐姿与站姿共用老师音色和教学资料；切换后使用该形象对应的课程动画。</p><div className="avatar-gallery-grid">{assets.filter(a=>a.kind==='image').map(a=><button type="button" key={a.id} className={`avatar-choice ${(editingTeacher||teacher)?.avatar_asset_id===a.id?'selected':''}`} aria-pressed={(editingTeacher||teacher)?.avatar_asset_id===a.id} onClick={()=>setEditingTeacher({...editingTeacher||teacher!,avatar_asset_id:a.id})}><img src={a.url} alt={(a.filename==='cartoon-teacher-v2.png'?'柔和写实（原版）':a.filename.replace(/\.[^.]+$/,''))} loading="lazy"/><span>{(a.filename==='cartoon-teacher-v2.png'?'柔和写实（原版）':a.filename.replace(/\.[^.]+$/,''))}</span><small>{(editingTeacher||teacher)?.avatar_asset_id===a.id?'已选择 · 保存后使用':'点击选择'}</small></button>)}</div></fieldset>
